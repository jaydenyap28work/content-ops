  ...
  return (
    <div className="page-enter space-y-6">
      <header className="grid gap-5 border-b border-line pb-6 lg:grid-cols-[1fr_auto] lg:items-end">
        <div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-coral">Production ledger</p><h2 className="mt-2 max-w-3xl font-display text-4xl font-semibold tracking-tight sm:text-5xl">Every approved direction becomes an accountable record.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-ink-muted">Content starts in Draft with a stable ID. Production workflow remains intentionally untouched until the next phase.</p></div>
        <div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setManagingCampaigns(true)}><CalendarRange className="size-4" />Campaigns</Button><Button onClick={() => setEditing(true)}><Plus className="size-4" />Create Content</Button></div>
      </header>
