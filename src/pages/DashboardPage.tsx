...
export function DashboardPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [eventRows, contentRows, queue] = await Promise.all([loadCalendarEvents(workspace.id, bounds.from, bounds.to), loadContents(workspace.id), loadAnalyticsQueue(workspace.id)])
    setEvents(eventRows); setContents(contentRows)
  ...
  return <div className="page-enter space-y-5">
    ...
    {loading ? <div className="grid min-h-72 place-items-center"><LoaderCircle className="size-7 animate-spin text-coral" /></div> : <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.55fr)]">
      ...
      <Card tone="dark"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral-light">{zh ? '制作概况' : 'Production overview'}</p><div className="mt-3 grid grid-cols-2 gap-2">{Object.entries(data.overview).map(([status, count]) => <button key={status} onClick={() => navigate(`/content?status=${status}`)} className="rounded-lg bg-white/7 p-3 text-left hover:bg-white/12"><p className="text-xl font-bold">{count}</p><p className="mt-1 text-xs text-white/60">{enumLabel(status, language)}</p></button>)}</div></Card></div>
    </div>}
  </div>
}