...
export function DashboardPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [eventRows, contentRows, queue, brand, taskRows] = await Promise.all([loadCalendarEvents(workspace.id, bounds.from, bounds.to), loadContents(workspace.id), loadAnalyticsQueue(workspace.id), loadBrandAccounts(workspace.id), loadTasks(workspace.id)])
    setEvents(eventRows); setContents(contentRows); setBrandAccounts(brand.accounts.filter((account) => account.is_active)); setBrandPlatforms(brand.platforms); setTasks(taskRows)
