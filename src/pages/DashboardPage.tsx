...
import type { PlatformRecord } from '../features/publishing/publishing-api'
import { loadTasks } from '../features/tasks/tasks-api'
import type { TaskRecord } from '../features/tasks/tasks-api'
import { loadEquipmentProposals } from '../features/equipment/equipment-api'
import type { EquipmentProposalRecord } from '../features/equipment/equipment-api'
import { countPendingAccessRequests } from '../features/admin/admin-api'
...
export function DashboardPage() {
  ...
  const [brandPlatforms, setBrandPlatforms] = useState<PlatformRecord[]>([])
  const [tasks, setTasks] = useState<TaskRecord[]>([])
  const [equipmentProposals, setEquipmentProposals] = useState<EquipmentProposalRecord[]>([])
  ...
  const refresh = useCallback(async () => {
    ...
    const isSuperAdmin = workspace.roles.includes('Super Admin')
    const [eventRows, contentRows, queue, brand, taskRows, proposalRows, accessCount] = await Promise.all([loadCalendarEvents(workspace.id, bounds.from, bounds.to), loadContents(workspace.id), loadAnalyticsQueue(workspace.id), loadBrandAccounts(workspace.id), loadTasks(workspace.id), loadEquipmentProposals(workspace.id), isSuperAdmin ? countPendingAccessRequests(workspace.id) : Promise.resolve(0)])
    setEvents(eventRows); setContents(contentRows); setBrandAccounts(brand.accounts.filter((account) => account.is_active)); setBrandPlatforms(brand.platforms); setTasks(taskRows); setEquipmentProposals(proposalRows); setPendingAccessRequests(accessCount)
  ...
  const socialOverview = useMemo(() => buildSocialOverview(brandPlatforms, brandAccounts), [brandAccounts, brandPlatforms])
  ...
  const pendingEquipment = equipmentProposals.filter((proposal) => proposal.status === 'pending_review')
  const allMyTasks = tasks.filter((task) => task.is_mine && task.status !== 'completed')
  const myTasks = allMyTasks.slice(0, 5)
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Kuala_Lumpur' }).format(new Date())
  const myDueToday = allMyTasks.filter((task) => task.due_date === today).length
  return <div className="page-enter space-y-5">
    ...
    {loading ? <div className="grid min-h-72 place-items-center"><LoaderCircle className="size-7 animate-spin text-coral" /></div> : <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.55fr)]">
      ...
      <div className="space-y-5">{isSuperAdmin && pendingAccessRequests > 0 ? <Card><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral">{zh ? '访问审批' : 'Access approval'}</p><h3 className="mt-1 font-bold">{zh ? `待审批 ${pendingAccessRequests} 人` : `${pendingAccessRequests} people waiting`}</h3></div><button onClick={() => navigate('/team')} className="text-xs font-bold text-blue">{zh ? '前往处理' : 'Review'}</button></div></Card> : null}{isSuperAdmin && pendingEquipment.length ? <Card><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral">{zh ? '待老板查看' : 'Boss review'}</p><h3 className="mt-1 font-bold">{zh ? `装备提案 ${pendingEquipment.length} 项` : `${pendingEquipment.length} equipment proposals`}</h3></div><button onClick={() => navigate('/equipment-proposals')} className="text-xs font-bold text-blue">{zh ? '查看提案' : 'Review proposals'}</button></div></Card> : null}<Card><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral">{zh ? '我的待办' : 'My Tasks'}</p><p className="mt-1 text-xs text-ink-muted">{zh ? `今天到期 ${myDueToday} · 待处理 ${myTasks.length}` : `Due today ${myDueToday} · Open ${myTasks.length}`}</p></div><button onClick={() => navigate('/tasks')} className="text-xs font-bold text-blue">{zh ? '快速新增' : 'Quick Add'}</button></div><div className="mt-3 divide-y divide-line">{myTasks.map(task => <button key={task.id} onClick={() => navigate('/tasks')} className="flex w-full items-center justify-between gap-3 py-2.5 text-left"><span className="line-clamp-2 text-sm font-bold">{task.title}</span><span className="shrink-0 text-xs text-ink-muted">{task.due_date ?? '—'}</span></button>)}{!myTasks.length ? <p className="py-4 text-sm text-ink-muted">{zh ? '目前没有分配给你的待办' : 'No tasks assigned to you'}</p> : null}</div></Card><Card><div className="flex items-center gap-2"><AlertTriangle className="size-5 text-gold-dark" /><h3 className="font-bold">{zh ? '需要处理' : 'Needs attention'}</h3></div><div className="mt-3 divide-y divide-line">{data.attention.map((item) => <button key={item.id} onClick={() => navigate(`/content/${item.id}`)} className="flex w-full items-center justify-between gap-3 py-3 text-left"><span className="line-clamp-2 text-sm font-bold">{item.title}</span><StatusBadge tone="warning">{enumLabel(item.current_status, language)}</StatusBadge></button>)}{publicationAttention.map((item, index) => <button key={`${item.contentId}-${item.type}-${index}`} onClick={() => navigate(`/content/${item.contentId}`)} className="flex w-full items-center justify-between gap-3 py-3 text-left"><span className="line-clamp-2 text-sm font-bold">{item.title}</span><StatusBadge tone="critical">{zh ? '发布 / 数据' : item.type}</StatusBadge></button>)}{!data.attention.length && !publicationAttention.length ? <p className="py-5 text-sm text-ink-muted">{zh ? '目前没有待审核、修改或发布异常' : 'No review, revision, or publication exceptions.'}</p> : null}</div></Card>
      ...
      <Card><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral">{zh ? '内部品牌' : 'Internal brand'}</p><h3 className="mt-1 font-bold">{zh ? 'LKSoft 账号概况' : 'LKSoft Account Overview'}</h3></div><button onClick={() => navigate('/brand/lksoft')} className="inline-flex items-center gap-1 text-xs font-bold text-blue">{zh ? '查看全部账号' : 'View all accounts'}<ArrowUpRight className="size-3.5" /></button></div><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2">{socialOverview.map(({ platform, account }) => <div key={platform.id} className="flex min-h-10 items-center justify-between gap-2 border-b border-line py-2 text-sm"><span className="truncate font-medium">{socialPlatformName(platform.code, platform.name, zh)}</span><span className={`font-mono font-bold tabular-nums ${account?.followers == null ? 'text-ink-faint' : 'text-ink'}`}>{formatFollowerCount(account?.followers ?? null, zh)}</span></div>)}</div>{!brandPlatforms.length ? <p className="mt-3 text-sm text-ink-muted">{zh ? '尚未记录真实账号资料' : 'No real account records yet.'}</p> : null}</Card></div>
    </div>}
  </div>
}