...
export function DashboardPage() {
  ...
  return <div className="page-enter space-y-5">
    ...
    {loading ? <div className="grid min-h-72 place-items-center"><LoaderCircle className="size-7 animate-spin text-coral" /></div> : <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.55fr)]">
      ...
      <Card><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.16em] text-coral">{zh ? '内部品牌' : 'Internal brand'}</p><h3 className="mt-1 font-bold">{zh ? 'LKSoft 账号概况' : 'LKSoft Account Overview'}</h3></div><button onClick={() => navigate('/brand/lksoft')} className="inline-flex items-center gap-1 text-xs font-bold text-blue">{zh ? '查看全部账号' : 'View all accounts'}<ArrowUpRight className="size-3.5" /></button></div><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2">{brandAccounts.slice(0, 6).map((account) => <div key={account.id} className="flex items-center justify-between gap-2 border-b border-line py-2 text-sm"><span className="truncate font-medium">{account.account_handle}</span><span className="font-mono font-bold">{account.followers == null ? (zh ? '待核对' : 'Pending') : new Intl.NumberFormat('en-MY', { notation: account.followers >= 10000 ? 'compact' : 'standard', maximumFractionDigits: 1 }).format(account.followers)}</span></div>)}</div>{!brandAccounts.length ? <p className="mt-3 text-sm text-ink-muted">{zh ? '尚未记录真实账号资料。' : 'No real account records yet.'}</p> : null}</Card></div>
    </div>}
  </div>
}