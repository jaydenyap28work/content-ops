...
import { GraphicContentWorkspace } from '../features/content/GraphicContentWorkspace'
...
import { ReviewActionPanel } from '../features/content/ReviewActionPanel'
import { ContentPresetPanel } from '../features/content/ContentPresetPanel'
import { ScriptWorkspace } from '../features/content/ScriptWorkspace'
import { ContentActivityLog } from '../features/content/ContentActivityLog'
...
export function ContentDetailPage() {
  ...
  const canEditProvider = isSuper || workspace?.roles.includes('Publisher / Marketing')
  ...
  const graphic = isGraphicFormat(content.content_format)
  ...
  const tabText: Record<Tab, string> = zh ? { shooting: graphic?'图文内容':'拍摄内容', progress: '制作进度', publishing: '发布' } : { shooting: graphic?'Graphic Content':'Shooting Pack', progress: 'Production Progress', publishing: 'Publishing' }
  ...
  return <div className="page-enter space-y-4">
    ...
    <header className="sticky top-0 z-20 rounded-xl border border-line bg-paper/95 px-5 py-4 shadow-sm backdrop-blur">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><StatusBadge tone="info">{enumLabel(content.current_status, language)}</StatusBadge></div><h1 className="mt-2 truncate text-2xl font-bold sm:text-3xl">{content.title}</h1><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-ink-muted">{!graphic?<span>{zh ? '计划拍摄日期' : 'Planned Shoot Date'}: <b className="text-ink">{formatWorkspaceDate(content.planned_shoot_date ?? null, language)}</b></span>:null}{content.planned_date ? <span>{zh ? '目标发布日期' : 'Target Publish Date'}: <b className="text-ink">{formatWorkspaceDate(content.planned_date, language)}</b></span> : null}<span>{zh ? '负责人' : 'Owner'}: <b className="text-ink">{content.current_owner_name ?? '—'}</b></span></div></div><div className="flex flex-wrap items-center gap-2">{!graphic?<Button variant="secondary" onClick={() => setBoss(true)} disabled={!sourceIdea}><Video className="size-4" />{zh ? '老板 / 拍摄模式' : 'Boss / Shooting Mode'}</Button>:null}{canManage ? <Button variant="secondary" onClick={() => setEditing(true)}><Pencil className="size-4" />{zh ? '编辑资料' : 'Edit metadata'}</Button> : null}<Button onClick={() => setTab(actionTab)}><Clapperboard className="size-4" />{tracker.nextAction}</Button></div></div>
    </header>
    ...
    {tab === 'shooting' ? graphic?<div className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]"><GraphicContentWorkspace content={content} clients={catalog.clients} canManage={Boolean(canManage)} onChanged={refresh}/><aside><ProductionTeamPanel workspaceId={workspace.id} clientId={content.client_id} contentId={content.id} contentFormat={content.content_format} contributors={workflow.contributors} ideaProviderName={sourceIdea?.provider_name??null} ideaProviderId={sourceIdea?.idea_provider_team_member_id??null} canManage={Boolean(canManage)} canEditProvider={Boolean(canEditProvider)} isPublished={content.current_status==='published'||content.current_status==='completed'} onChanged={refresh}/></aside></div>:<div className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]"><Card className="min-w-0">{sourceIdea ? <ShootingBrief idea={sourceIdea} references={sourceReferences} team={workflow.contributors.filter(item => item.status === 'active').map(item => ({ roleCode: item.contribution_role_code, name: item.display_name }))} /> : <div className="py-12 text-center text-ink-muted">{zh ? '这个内容没有来源选题；可在更多资料建立脚本版本' : 'This Content has no source Idea. Script versions remain under More.'}</div>}<details className="mt-6 border-t border-line pt-4"><summary className="cursor-pointer font-bold text-blue">{zh ? '更多 · 版本记录' : 'More · Version history'}</summary><div className="mt-4"><ScriptWorkspace contentId={content.id} scripts={review.scripts} canManage={Boolean(canManage)} onChanged={refresh} /></div></details></Card><aside className="space-y-4 xl:sticky xl:top-48 xl:self-start"><ProductionTeamPanel workspaceId={workspace.id} clientId={content.client_id} contentId={content.id} contentFormat={content.content_format} contributors={workflow.contributors} ideaProviderName={sourceIdea?.provider_name??null} ideaProviderId={sourceIdea?.idea_provider_team_member_id??null} canManage={Boolean(canManage)} canEditProvider={Boolean(canEditProvider)} isPublished={content.current_status==='published'||content.current_status==='completed'} onChanged={refresh} />{sourceIdea ? <ShootingSchedulePanel ideaId={sourceIdea.id} clientId={content.client_id} plannedShootDate={content.planned_shoot_date ?? null} scheduledAt={workflow.production.shoot_scheduled_at} /> : null}</aside></div> : null}

    {tab === 'progress' ? <div className="space-y-5">{!graphic?<><ProductionWorkspace key={`${content.id}-${content.updated_at}`} workspaceId={workspace.id} content={content} bundle={workflow} currentUserId={session.user.id} workspaceRoles={workspace.roles} canManage={Boolean(canManage)} onChanged={refresh} /><ReviewActionPanel content={content} bundle={review} contributors={workflow.contributors} currentUserId={session.user.id} workspaceRoles={workspace.roles} onChanged={refresh}/><ContentPresetPanel contentId={content.id} clientId={content.client_id} canManage={Boolean(canManage)}/></>:null}<ReviewSummary review={review} workflow={workflow} language={language} /><details className="rounded-xl border border-line bg-paper"><summary className="flex cursor-pointer list-none items-center gap-2 px-5 py-4 font-bold"><MoreHorizontal className="size-4 text-coral" />{zh ? '高级审核设置与完整修改记录' : 'Advanced review settings and full revision history'}</summary><div className="border-t border-line p-5"><ReviewWorkspace content={content} bundle={review} contributors={workflow.contributors} currentUserId={session.user.id} workspaceRoles={workspace.roles} canManage={Boolean(canManage)} onChanged={refresh} /></div></details></div> : null}
    ...
    <details className="rounded-xl border border-line bg-paper"><summary className="flex cursor-pointer list-none items-center gap-2 px-5 py-4 font-bold"><MoreHorizontal className="size-4 text-coral" />{zh ? '更多资料' : 'More details'}</summary><div className="space-y-5 border-t border-line p-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-ink-muted">{zh ? '内容编号' : 'Content Code'}</p><p className="mt-1 font-mono text-xs">{content.content_code}</p></div><div><p className="text-xs text-ink-muted">{zh ? '内部备注' : 'Internal notes'}</p><p className="mt-1 text-sm">{content.internal_notes || '—'}</p></div><div><p className="text-xs text-ink-muted">{zh ? '剪辑负责人' : 'Editor'}</p><p className="mt-1 font-bold">{contributor('editor')}</p></div><div><p className="text-xs text-ink-muted">{zh ? '发布负责人' : 'Publisher'}</p><p className="mt-1 font-bold">{contributor('publisher')}</p></div></div><ContentActivityLog workflow={workflow} review={review} />{isSuper ? <Button variant="danger" onClick={() => void handleArchive()}><Archive className="size-4" />{zh ? '归档内容' : 'Archive Content'}</Button> : null}</div></details>
