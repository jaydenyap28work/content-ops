...
export function ContentDetailPage() {
  ...
  return <div className="page-enter space-y-4">
    ...
    <header className="sticky top-0 z-20 rounded-xl border border-line bg-paper/95 px-5 py-4 shadow-sm backdrop-blur">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><StatusBadge tone="info">{enumLabel(content.current_status, language)}</StatusBadge><span className="text-xs font-bold text-ink-muted">{content.ownership_name}</span></div><h1 className="mt-2 truncate text-2xl font-bold sm:text-3xl">{content.title}</h1><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-ink-muted"><span>{zh ? '计划拍摄日期' : 'Planned Shoot Date'}: <b className="text-ink">{formatWorkspaceDate(content.planned_shoot_date ?? null, language)}</b></span>{content.planned_date ? <span>{zh ? '目标发布日期' : 'Target Publish Date'}: <b className="text-ink">{formatWorkspaceDate(content.planned_date, language)}</b></span> : null}<span>{zh ? '负责人' : 'Owner'}: <b className="text-ink">{content.current_owner_name ?? '—'}</b></span></div></div><div className="flex flex-wrap items-center gap-2"><Button variant="secondary" onClick={() => setBoss(true)} disabled={!sourceIdea}><Video className="size-4" />{zh ? '老板 / 拍摄模式' : 'Boss / Shooting Mode'}</Button>{canManage ? <Button variant="secondary" onClick={() => setEditing(true)}><Pencil className="size-4" />{zh ? '编辑资料' : 'Edit metadata'}</Button> : null}<Button onClick={() => setTab(actionTab)}><Clapperboard className="size-4" />{tracker.nextAction}</Button></div></div>
    </header>
    ...
    {tab === 'shooting' ? <div className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]"><Card className="min-w-0">{sourceIdea ? <ShootingBrief idea={sourceIdea} references={sourceReferences} /> : <div className="py-12 text-center text-ink-muted">{zh ? '这个内容没有来源选题；可在更多资料建立脚本版本。' : 'This Content has no source Idea. Script versions remain under More.'}</div>}<details className="mt-6 border-t border-line pt-4"><summary className="cursor-pointer font-bold text-blue">{zh ? '更多 · 版本记录' : 'More · Version history'}</summary><div className="mt-4"><ScriptWorkspace contentId={content.id} scripts={review.scripts} canManage={Boolean(canManage)} onChanged={refresh} /></div></details></Card><aside className="space-y-4 xl:sticky xl:top-48 xl:self-start"><Card><p className="text-xs font-extrabold uppercase tracking-[.18em] text-coral">{zh ? '拍摄安排' : 'Shoot logistics'}</p><dl className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-1">{[[zh ? '品牌' : 'Brand', content.ownership_name], [zh ? '分类' : 'Category', category?.name ?? '—'], [zh ? '出镜' : 'Talent', '—'], [zh ? '拍摄负责人' : 'Shooter', contributor('shooter')], [zh ? '计划拍摄' : 'Planned shoot', formatWorkspaceDate(content.planned_shoot_date ?? null, language)], [zh ? '具体时间' : 'Time', dateTime(workflow.production.shoot_scheduled_at, language)]].map(([key, value]) => <div key={key}><dt className="text-xs text-ink-muted">{key}</dt><dd className="mt-1 text-sm font-bold">{value}</dd></div>)}</dl></Card></aside></div> : null}
    ...
    <details className="rounded-xl border border-line bg-paper"><summary className="flex cursor-pointer list-none items-center gap-2 px-5 py-4 font-bold"><MoreHorizontal className="size-4 text-coral" />{zh ? '更多资料' : 'More details'}</summary><div className="space-y-5 border-t border-line p-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-ink-muted">Content Code</p><p className="mt-1 font-mono text-xs">{content.content_code}</p></div><div><p className="text-xs text-ink-muted">{zh ? '内部备注' : 'Internal notes'}</p><p className="mt-1 text-sm">{content.internal_notes || '—'}</p></div><div><p className="text-xs text-ink-muted">{zh ? '剪辑负责人' : 'Editor'}</p><p className="mt-1 font-bold">{contributor('editor')}</p></div><div><p className="text-xs text-ink-muted">{zh ? '发布负责人' : 'Publisher'}</p><p className="mt-1 font-bold">{contributor('publisher')}</p></div></div><TimelineWorkspace bundle={workflow} />{isSuper ? <Button variant="danger" onClick={() => void handleArchive()}><Archive className="size-4" />{zh ? '归档内容' : 'Archive Content'}</Button> : null}</div></details>
