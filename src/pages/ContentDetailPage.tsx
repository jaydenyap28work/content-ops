...
export function ContentDetailPage() {
  ...
  const hasContentAccess = isSuperAdmin || isManager || workspace?.roles.includes('Strategist / Content Planner')
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setDetail(nextDetail); setCatalog(nextCatalog)
    ...
    <div className="page-enter space-y-6">
      ...
      <header className="relative overflow-hidden rounded-xl border border-white/10 bg-ink p-6 text-paper shadow-xl sm:p-8">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2">{content.record_status === 'active' ? <Button variant="secondary" onClick={() => setEditing(true)}><Pencil className="size-4" />Edit metadata</Button> : null}{content.record_status === 'active' && (isSuperAdmin || isManager) ? <Button variant="ghost" className="text-white/65 hover:bg-white/10 hover:text-white" disabled={busy} onClick={() => void archive()}><Archive className="size-4" />Archive</Button> : null}</div>
        </div>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <aside className="space-y-4 xl:sticky xl:top-28 xl:self-start">
          <Card tone="quiet"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-coral">Current context</p><dl className="mt-4 space-y-4"><div><dt className="text-xs text-ink-muted">Production state</dt><dd className="mt-1 font-bold">{label(content.current_status)}</dd></div><div><dt className="text-xs text-ink-muted">Primary next action</dt><dd className="mt-1 text-sm font-semibold">Prepare Content brief / Script</dd></div><div><dt className="text-xs text-ink-muted">Last updated</dt><dd className="mt-1 text-sm font-semibold">{new Date(content.updated_at).toLocaleString('en-MY')}</dd></div></dl><p className="mt-5 border-t border-line pt-4 text-xs leading-5 text-ink-muted">Production transitions, shooting, editing, approvals, and publications are intentionally unavailable until their roadmap phases.</p></Card>
      ...
      {editing && workspace ? <ContentFormDrawer workspaceId={workspace.id} catalog={catalog} content={content} canManagePrivateNotes={isSuperAdmin || isManager} onClose={() => setEditing(false)} onSaved={async () => { setEditing(false); await refresh() }} /> : null}
    </div>
  )
}
