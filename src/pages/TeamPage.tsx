...
export function TeamPage() {
  ...
  const [invite, setInvite] = useState({ email: '', displayName: '', jobTitle: '', roleIds: [] as string[] })
  ...
  async function handleInvite(event: FormEvent) {
    ...
    setInviteOpen(false)
    setInvite({ email: '', displayName: '', jobTitle: '', roleIds: [] })
  }
  ...
  return (
    <div className="page-enter space-y-6">
      ...
      {inviteOpen ? <div className="fixed inset-0 z-50 flex justify-end bg-ink/45 backdrop-blur-sm"><section className="h-full w-full max-w-lg overflow-y-auto bg-paper p-6 shadow-2xl sm:p-8"><div className="flex justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-coral">Secure invitation</p><h3 className="mt-2 font-display text-3xl font-semibold">Invite Team member</h3></div><Button variant="ghost" size="icon" onClick={() => setInviteOpen(false)}><X className="size-5" /></Button></div><p className="mt-4 text-sm leading-6 text-ink-muted">Supabase sends the invitation. ContentOS provisions only the selected predefined Workspace roles; public registration remains disabled.</p><form className="mt-7 space-y-5" onSubmit={handleInvite}><FormField label="Work email" htmlFor="invite-email" required><Input id="invite-email" type="email" required value={invite.email} onChange={(e) => setInvite({ ...invite, email: e.target.value })} /></FormField><FormField label="Display name" htmlFor="invite-name" required><Input id="invite-name" required value={invite.displayName} onChange={(e) => setInvite({ ...invite, displayName: e.target.value })} /></FormField><FormField label="Job title" htmlFor="invite-title"><Input id="invite-title" value={invite.jobTitle} onChange={(e) => setInvite({ ...invite, jobTitle: e.target.value })} /></FormField><fieldset className="space-y-2"><legend className="mb-2 text-sm font-semibold">Initial roles <span className="text-coral">*</span></legend>{roles.map((role) => <label key={role.id} className="flex gap-3 rounded-md border border-line p-3 text-sm font-bold"><input type="checkbox" className="size-4 accent-coral" checked={invite.roleIds.includes(role.id)} onChange={(e) => setInvite({ ...invite, roleIds: e.target.checked ? [...invite.roleIds, role.id] : invite.roleIds.filter((id) => id !== role.id) })} />{role.name}</label>)}</fieldset><div className="flex gap-3 border-t border-line pt-5"><Button type="submit" disabled={busy || invite.roleIds.length === 0}>{busy ? <LoaderCircle className="size-4 animate-spin" /> : null}Send invitation</Button><Button variant="secondary" onClick={() => setInviteOpen(false)}>Cancel</Button></div></form></section></div> : null}
    </div>
  )
}
