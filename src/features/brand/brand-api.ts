...
export interface BrandSocialAccount extends SocialAccountRecord { followers:number|null;followers_updated_at:string|null;note:string|null;updated_at:string }
...
export async function loadBrandBundle(workspaceId:string):Promise<BrandBundle>{
  ...
  const[platforms,accounts,assets]=await Promise.all([
    supabase.from('platforms').select('id,code,name').in('code',['facebook','instagram','youtube','xhs','threads','lemon8']).eq('is_active',true).order('sort_order'),
    supabase.from('social_accounts').select('id,client_id,platform_id,account_name,account_handle,external_url,is_active,followers,followers_updated_at,note,updated_at').eq('client_id',brand.id).order('account_name'),
