...
export interface ShootingBrief{idea_id:string;why_now:string|null;interview_questions:string[];key_talking_points:string[];suggested_cta:string|null;target_duration:string|null;talent:string|null;shoot_date:string|null;location:string|null;shooter_user_id:string|null}
export async function loadShootingBrief(ideaId:string){const{data,error}=await supabase.from('idea_shooting_briefs').select('idea_id,why_now,interview_questions,key_talking_points,suggested_cta,target_duration,talent,shoot_date,location,shooter_user_id').eq('idea_id',ideaId).maybeSingle();fail(error);return data as ShootingBrief|null}
export async function saveShootingBrief(ideaId:string,brief:Omit<ShootingBrief,'idea_id'>){const{error}=await supabase.rpc('save_idea_shooting_brief',{target_idea_id:ideaId,target_why_now:brief.why_now??'',target_interview_questions:brief.interview_questions,target_key_talking_points:brief.key_talking_points,target_suggested_cta:brief.suggested_cta??'',target_target_duration:brief.target_duration??'',target_talent:brief.talent??'',target_shoot_date:brief.shoot_date||null,target_location:brief.location??'',target_shooter_user_id:brief.shooter_user_id||null});fail(error)}
