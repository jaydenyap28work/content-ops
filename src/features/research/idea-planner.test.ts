...
function idea(overrides: Partial<IdeaRecord>): IdeaRecord {
  return {
    id: 'idea', workspace_id: 'workspace', client_id: 'client', title: 'Idea', planned_date: null, shoot_planned_at: null,
    source_url: null, original_topic: null, original_hook: null, why_it_works: null, our_angle: null,
    category_id: null, suggested_format: null, priority: 'normal', status: 'new', planning_status: 'new', owner_user_id: null,
    ...
    linked_content_planned_date: null, linked_content_shoot_scheduled_at: null, notes: null, status_reason: null, created_at: '2026-08-20',
...
describe('Idea Planner helpers', () => {
  ...
  it('filters by the human workflow status without conflating Content status', () => {
    const rows = [idea({ id: 'new' }), idea({ id: 'confirmed', status: 'converted', planning_status: 'confirmed', linked_content_status: 'editing' })]
    const filtered = filterPlannerIdeas(rows, { search: '', status: 'confirmed', clientId: 'all', categoryId: 'all', reference: 'all' })
    expect(filtered.map((item) => item.id)).toEqual(['confirmed'])
  })

  it('shows the linked production status for a converted Idea', () => {
    expect(getDisplayedProductionStatus(idea({ status: 'converted', planning_status: 'confirmed', linked_content_status: 'ready_to_shoot' }))).toBe('Awaiting Shoot')
